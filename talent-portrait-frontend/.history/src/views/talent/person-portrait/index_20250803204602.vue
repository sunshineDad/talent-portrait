    ...
    series: [
      {
        ...
        data: [
          {
            value: radarData.map((item: any) => item.score),
            name: '能力评估',
            symbol: 'circle',
            symbolSize: 6,
            lineStyle: {
...
const initInnovationChart = () => {
  const chartDom = document.querySelector('.innovation-output .chart-container') as HTMLElement
  if (!chartDom) return
  ...
  innovationChartInstance = echarts.init(chartDom)

  const yearData = portraitData.value.innovationData.yearData
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: yearData.map((item: any) => item.year),
      axisLine: {
        lineStyle: {
          color: 'rgba(32, 168, 216, 0.6)',
        },
    ...
    series: [
      {
        ...
        data: yearData.map((item: any) => item.patent || 0),
      },
      {
        name: '论文',
        type: 'bar',
        ...
        data: yearData.map((item: any) => item.paper || 0),
      },
      {
        name: '标准',
        type: 'bar',
        ...
        data: yearData.map((item: any) => item.standard || 0),
      },
      {
        name: '软著',
        type: 'bar',
        ...
        data: yearData.map((item: any) => item.software || 0),
      },
    ],
  }
